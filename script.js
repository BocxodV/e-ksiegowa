...
function changeLanguage() {
  ...
  const cardInput = document.getElementById("cardAmount");
    if (cardInput) {
        cardInput.placeholder = TRANSLATIONS[selectedLang].placeholder_card;
    }
}
...
if (gTarget > 0 && cSav > 0) {
    percent = Math.min((cSav / gTarget) * 100, 100);
} else {
    percent = 0;
}
...
if (urlParams.has("g_dead")) document.getElementById("goalDeadlineInput").value = urlParams.get("g_dead");
...
// --- НОВОЕ: НАПОЛНЕНИЕ ВЫПАДАЮЩИХ СПИСКОВ (СПРИНТ 2) ---
const rawLocs = urlParams.get('locs') || "";
const rawCars = urlParams.get('cars') || "";

function populateDatalist(listId, dataString) {
    const list = document.getElementById(listId);
    if (!list || !dataString) return;

    // Декодируем и разбиваем строку обратно в массив
    const items = decodeURIComponent(dataString).split(',');
...
populateDatalist('locationsList', rawLocs);
populateDatalist('carsList', rawCars);
// --------------------------------------------------------
...
function openTab(tabId) {
  ...
  // 1. Убираем активные классы и старую анимацию
  ...
  // 3. Запускаем стабильную и легкую анимацию проявления
  setTimeout(() => {
    targetContent.classList.add("page-fade");
  }, 10);

  // Оставляем приятную вибрацию при переключении
...
function handleSwipe() {
    ...
    if (swipeDistance < -threshold) {
        if (currentIndex < tabsOrder.length - 1) openTab(tabsOrder[currentIndex + 1]);
    }
    if (swipeDistance > threshold) {
        if (currentIndex > 0) openTab(tabsOrder[currentIndex - 1]);
    }
}